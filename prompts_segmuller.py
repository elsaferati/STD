...
def build_user_instructions_segmuller(source_priority: list[str]) -> str:
    return (
        ...
        "From Furnplan rows:\n"
        "- HARD RULE (apply first): if ArtNr ends with artikel token ^\\d{5}[A-Z]?$, this token is artikelnummer.\n"
        "- modellnummer is the remaining ArtNr prefix before that token (strip separators/spaces).\n"
        "- This rule applies to simple forms like IN446144SP36-42187, ZB00-46518, ZB9191/64151, SI9191XP04695.\n"
        ...
        "- Only leave artikelnummer empty when no ArtNr trailing token can be read at all.\n"
        "- Do NOT write warnings claiming 'no 5-digit artikelnummer' when ArtNr clearly ends with such a token.\n"
