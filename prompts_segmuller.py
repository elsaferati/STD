...
def build_user_instructions_segmuller(source_priority: list[str]) -> str:
    return (
        ...
        "- Primary BESTELLUNG pages define the authoritative ordered Seg-Nr scope for the order.\n"
        "\n"
        "=== SEGMULLER VENDOR SECTION FILTER ===\n"
        "On furnplan/scanned pages, detect section headers like '<firm> <program> (Seg.Nr. 2148807)'.\n"
        "Treat each firm section as a separate subdocument.\n"
        "Extract rows ONLY from sections headed by Staud.\n"
        "Ignore sections headed by any other firm, including OCR variants like Wiemman/Wiemann.\n"
        "If a page mixes multiple firms, do not merge rows across vendor sections.\n"
        "Keep row order only within the retained Staud section(s).\n"
        "Cross-check retained Staud sections against the ordered Seg-Nr values from BESTELLUNG pages.\n"
        "When multiple vendor sections exist, prefer the Staud section whose Seg.Nr. matches the ordered item scope.\n"
        "Do not extract rows from unmatched or non-Staud sections even if they appear earlier on the page.\n"
        "If no Staud section is visible, return no items from that furnplan block.\n"
        "\n"
        ...
        "- Extract ALL Staud items from ALL pages - don't stop after first retained table!\n"
        "\n"
        "=== ROW EXTRACTION ===\n"
