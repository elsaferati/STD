...
def build_user_instructions_porta(source_priority: list[str]) -> str:
    return (
        ...
        "5) Do not keep combined token in one field when it clearly splits.\n"
        "6) Preserve leading zeros and uppercase letters exactly.\n"
        ...
        "\n"
        "=== FURNCLOUD ID ===\n"
        "Look for bracketed codes like '[abcd ef12]' anywhere on any PDF page (including rotated/side margins).\n"
        "Scan ALL pages, even image/drawing pages with no item table; the furncloud ID may only appear there.\n"
        "Before final output, perform a full-page sweep for each PDF page in order: main body first, then all four margins/side strips.\n"
        "Do not finalize furncloud_id until all PDF pages have been checked.\n"
        "On Porta documents, furncloud ID is often on page 2 near Kommission/Kunde/date and printed vertically/rotated in a side strip.\n"
        "Treat that side-strip code as valid even when no item table exists on that page.\n"
        "It can be printed vertically/rotated in a side margin or small box near the date/commission area.\n"
        "Read candidate text in any orientation (0, 90, 180, 270 degrees).\n"
        "Accept vertical text where characters are stacked or run along the page margin.\n"
        "Also accept text label pattern like 'furncloud: (abcd ef12)' in email or PDF text.\n"
        "If the code appears only in the page image (not in digital text), still extract it from the image.\n"
        "Preserve exact characters and spacing; do not normalize O vs 0.\n"
        "Prefer using digital text to confirm characters when available.\n"
        "Valid furncloud_id is exactly 8 alphanumeric characters. The stored format must be 'XXXX XXXX' "
        "(4 chars, space, 4 chars).\n"
        "If the source shows 8 chars with no space (e.g., 'abcd1234'), accept it and normalize to 'abcd 1234'.\n"
        "If the source shows more than 8 alphanumeric characters (e.g., 'mzwq gkin 9'), keep only the first 8 "
        "alphanumeric characters in reading order and normalize to 'mzwq gkin'. Ignore any trailing characters.\n"
        "Do not require item-table context for furncloud extraction; margin-only occurrence is sufficient.\n"
        "If found once, apply the same furncloud_id to all items.\n"
        "If email has no furncloud pattern, prioritize a bracketed candidate found in PDF page image side/margins instead of leaving empty.\n"
        "If multiple candidates appear, use the first in reading order: page order first (1, 2, 3, ...), then within a page use main content top-to-bottom, then margin/side-strip top-to-bottom.\n"
