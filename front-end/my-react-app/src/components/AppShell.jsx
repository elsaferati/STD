        ...
        <aside className="hidden lg:flex w-64 flex-col bg-[#EEF1F4] text-slate-800 relative overflow-hidden sticky top-0 h-screen shadow-[6px_0_6px_rgba(15,23,42,0.08)] border-r border-slate-200/80">
          ...
          <div className="relative z-10 px-6 py-6 border-b border-slate-300/80">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm uppercase tracking-[0.2em] text-slate-600">Operations</p>
          ...
          <div className="relative z-10 flex-1 px-4 py-6 space-y-6 overflow-y-auto">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-slate-600 mb-3">Navigation</p>
              <div className="space-y-2">
                <NavLink to="/" active={active === "overview"} icon="space_dashboard" label="Overview" />
                <NavLink to="/orders" active={active === "orders"} icon="receipt_long" label="Orders" />
            ...
            {showPulse ? (
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-slate-600 mb-3">Pulse</p>
                <div className="rounded-xl border border-slate-300 bg-slate-100 p-4 space-y-3">
                  <div>
                    <p className="text-xs text-slate-600">Queue health</p>
                    <p className="text-lg font-semibold text-slate-900">{pulseValue} pending</p>
                  </div>
                  <div className="h-1.5 rounded-full bg-slate-300 overflow-hidden">
                    <div className="h-full bg-primary/80" style={{ width: `${percent}%` }} />
                  </div>
                  <p className="text-[11px] text-slate-600">Based on today&apos;s total inflow.</p>
                </div>
              </div>
            ) : null}
            ...
            {sidebarContent ? (
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-slate-500 mb-3">Filters</p>
                {sidebarContent}
              </div>
            ) : null}
          </div>
