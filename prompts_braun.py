...
def build_user_instructions_braun(source_priority: list[str]) -> str:
    return (
        ...
        "\n"
        "--- FORMAT F — Ausf. model code + 'Type XXXXX' article ---\n"
        "  Trigger: 'Ausf.' is followed IMMEDIATELY by an alphanumeric code as its first token\n"
        "  (e.g., 'Ausf. PD916191SP91 Außentüren Dekor sand').\n"
        "  AND/OR a 'Type XXXXX' line (English spelling, no colon) appears in the block.\n"
        "\n"
        "  - modellnummer  = the code immediately after 'Ausf.' (first token, e.g., 'PD916191SP91')\n"
        "  - artikelnummer = the code after 'Type ' (e.g., '78705' from 'Type 78705, 3trg.')\n"
        "  - menge         = quantity from the main position line\n"
        "  - furncloud_id  = '' (empty)\n"
        "\n"
        "  CRITICAL: The Ausf. code is the modellnummer ONLY when it appears as the first token\n"
        "  after 'Ausf.' with no preceding color/material words. If 'Ausf.' is followed by a\n"
        "  color description (e.g., 'Ausf. Dekor weiß, CQ9696'), the trailing code is a color\n"
        "  designator — NOT the modellnummer.\n"
        "\n"
        "  'Type' (English) is treated identically to 'Typ:' (German with colon).\n"
        "\n"
        "  Example from PDF (fax00670353):\n"
        ...
        "  Trigger: No Art.Nr., no '[N]x Typ', no 'Typ:' line, no 'bestehend aus:',\n"
        "  no 'Type ' line under the position.\n"
        ...
        "  UNLESS 'Ausf.' is immediately followed by an alphanumeric model code (Format F).\n"
