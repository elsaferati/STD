...
def build_user_instructions_braun(source_priority: list[str]) -> str:
    return (
        ...
        "\n"
        "  - modellnummer  = extracted as described in F1 or F2 above\n"
        ...
        "  - furncloud_id  = '' (empty)\n"
        "\n"
        "  CRITICAL: Sub-case F1 applies only when the first token after 'Ausf.'/'Ausf.:'\n"
        "  is an alphanumeric identifier (contains both letters and digits, OR is an\n"
        "  all-caps/mixed-case product code). If the first words are plain German words like\n"
        "  'bettrahmenfarbe', fall through to other formats (not F1). Sub-case F2 handles\n"
        "  the remaining case where a color word comes first but a code appears later.\n"
        "  Examples that are NOT F1 (but F2 or other):\n"
        "    'Ausf. Dekor weiß, CQ9696'        <- F2: trailing code CQ9696 is modellnummer\n"
        "    'Ausf.: bettrahmenfarbe = Dekor …' <- neither F1 nor F2 → skip Ausf. entirely\n"
        "\n"
        ...
        "  - artikelnummer = the 5-digit code after 'Art.' (e.g., '53879', '30155')\n"
        "  - modellnummer  = empty string '' (post-processing Zubehör catalog will fill it)\n"
        "  - menge         = 1 (default for accessories unless a quantity is explicitly stated)\n"
        "  - furncloud_id  = '' (empty)\n"
        ...
        "  Example:\n"
        "    '1  Schwebetürenschrank  Sigma Lucca'       (right-aligned: 842308 — ignore)\n"
        "    'Art. OGA67120-12692'                       -> primary item (Format D)\n"
        "    'Art. 53879 Schubkasteneinsatz ...'         -> additional item (G), artikelnummer='53879'\n"
        "    'Art. 30155 Öffnungs- und Schließdämpfer'  -> additional item (G), artikelnummer='30155'\n"
        "    Both accessory items: modellnummer='', menge=1, furncloud_id=''\n"
        "\n"
        ...
        "  If none of the above formats (D, F, E, A, B, H) match for a position:\n"
        ...
        "  Lines starting with '[N]x [descriptor]' that do NOT contain 'Typ' AND do NOT\n"
        "  contain a FORMAT D hyphenated code (CODE-CODE) are product attributes / included\n"
        "  accessories — NOT separate order items.\n"
        "  Examples: '3x Fachboeden', '1x T-Einteilung', '1x Schubkaseneinsatz, 2 Schubkasten'\n"
        "  EXCEPTION: '[N]x Description CODE-CODE' lines (Format D3) ARE separate items.\n"
        "  DO NOT create output items for plain accessory lines without a D-format code.\n"
        "\n"
